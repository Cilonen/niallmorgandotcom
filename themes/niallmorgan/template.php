...
// Add Zen Tabs styles
if (theme_get_setting('niallmorgan_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'niallmorgan') .'/css/tabs.css');
}

function niallmorgan_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . niallmorgan_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = niallmorgan_id_safe('page-'. $path);
    $vars['classes_array'][] = niallmorgan_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('niallmorgan_ie_enabled')) {
    $niallmorgan_ie_enabled_versions = theme_get_setting('niallmorgan_ie_enabled_versions');
    if (in_array('ie8', $niallmorgan_ie_enabled_versions, TRUE)) {
...
function niallmorgan_preprocess_page(&$vars, $hook) {
...
function niallmorgan_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from niallmorgan_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function niallmorgan_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function niallmorgan_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('niallmorgan_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('niallmorgan_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('niallmorgan_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('niallmorgan_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('niallmorgan_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function niallmorgan_id_safe($string) {
 ...
 *
 */
function niallmorgan_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = niallmorgan_id_safe($element['#title']);
 ...
 */
function niallmorgan_preprocess_menu_local_task(&$variables) {
 ...
 */
function niallmorgan_menu_local_tasks(&$variables) {
  $output = '';
