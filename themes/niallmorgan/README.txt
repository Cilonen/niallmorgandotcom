...
Introduction to niallmorgan

niallmorgan boasts a clean HTML5 structure with extensible CSS classes and ID's for unlimited
...
niallmorgan's goal is to provide themers the building blocks needed to get their designs up and
running quickly and simply.

niallmorgan is perfect if you want a simple, smart, and flexible theme starter.
...
- niallmorgan utilizes SASS for adaptive grids and layouts and general structure of the site.
...
- niallmorgan is meant to be YOUR theme. To change the name of the theme from 'niallmorgan' to another name like 'mytheme',
follow these steps (to do BEFORE enabling the theme) :

    - rename the theme folder to 'mytheme'
    - rename niallmorgan.info to mytheme.info
    - Edit niallmorgan.info and change the name, description, project (can be deleted), replace all other instances of "niallmorgan" ie. [niallmorgan_tabs] would become [mytheme_tabs]
    - In template.php change each iteration of 'niallmorgan' to 'mytheme'
    - In theme-settings.php change each iteration of 'niallmorgan' to 'mytheme'
...
- niallmorgan.info                => provide informations about the theme, like regions, css, settings, js ...
...
The layout used in niallmorgan is fairly similar to the Holy Grail method. It has been tested on
...
This is how the page template is buit in niallmorgan, and it works in fluid and fixed layout.
Refers to the notes in layout.css to see how to modify the layout.

__________________________________________________________________________________________

UPDATING niallmorgan

Once you start using niallmorgan, you will massively change it until a point where it has nothing
to do with niallmorgan anymore. Unlike ZEN, niallmorgan is not intended to be use as a base theme for a
sub-theme (even though it is possible to do so). Because of this, it is not necessary to
update your theme when a new version of niallmorgan comes out. Always see niallmorgan as a STARTER, and
as soon as you start using it, it is not niallmorgan anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when niallmorgan has a new version
by the update module, simply delete "project = "niallmorgan" in niallmorgan.info

__________________________________________________________________________________________

Thanks for using niallmorgan, and remember to use the issue queue in drupal.org for any question
or bug report:

http://drupal.org/project/issues/niallmorgan
